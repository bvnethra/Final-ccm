  ...
  const filteredLabQueue = useMemo(() => {
    let result = allRequests.filter((r) => {
      ...
      if (activeStageTab === 'OUTSOURCED')
        return (
          r.status === 'OUTSOURCED' ||
          r.status === 'OUTSOURCE_RETURNED' ||
          Boolean(r.request_items?.some((it) => it.destination === 'VENDOR_OUTSOURCE'))
        );
                  ...
                  <td colSpan={8} className="px-5 py-16 text-center text-slate-500">
                    <div className="max-w-md mx-auto space-y-3">
                      ...
                      <h3 className="font-semibold text-base text-slate-900">
                        {activeStageTab === 'OUTSOURCED'
                          ? 'No Outsource PO Orders Found'
                          : activeStageTab === 'IN_REPAIR'
                          ? 'No Repair Orders Found'
                          : activeStageTab === 'PENDING_VERIFY'
                          ? 'No Pending Verification Orders'
                          : activeStageTab === 'PENDING_CALIBRATE'
                          ? 'No Pending Calibration Orders'
                          : 'No Work Orders Found'}
                      </h3>
                      <p className="text-xs text-slate-500">
                        {activeStageTab !== 'LAB_ALL' || priorityFilter !== 'ALL' || searchQuery
                          ? `No work orders currently match the "${
                              activeStageTab === 'OUTSOURCED'
                                ? 'Outsource PO'
                                : activeStageTab === 'IN_REPAIR'
                                ? 'Repair'
                          ...
                          : 'All received customer equipment has completed the lab queue.'}
                      </p>
                      {(activeStageTab !== 'LAB_ALL' || priorityFilter !== 'ALL' || searchQuery) && (
                        <div className="pt-2">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#0274BB] hover:bg-[#003B8C] text-white text-xs font-semibold shadow-xs transition-colors cursor-pointer"
                          >
                            View All Orders ({allRequests.length})
                          </button>
                        </div>
                      )}
                    </div>
