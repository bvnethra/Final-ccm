...
export default function TenantDetailPage() {
  ...
  if (error || !tenant) {
    return (
      <div className="p-6 rounded-lg bg-red-950/20 border border-red-800/40 text-red-300 text-sm max-w-lg mx-auto text-center space-y-4">
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-xl font-bold text-zinc-100 tracking-tight">{tenant.name}</h1>
              <Badge variant={tenant.status === 'ACTIVE' ? 'success' : 'destructive'}>
                {tenant.status}
              </Badge>
            </div>
            <p className="text-xs text-zinc-400 font-mono mt-0.5">
              Code: <strong className="text-zinc-200">{tenant.code}</strong> &bull; ID: {tenant.id}
            </p>
          </div>
        </div>
      ...
      {feedbackMsg && (
        <div className="p-3 rounded-md bg-emerald-950/40 border border-emerald-800/40 text-emerald-300 text-xs flex items-center justify-between animate-fadeIn">
          ...
          <button onClick={() => setFeedbackMsg('')} className="text-zinc-400 hover:text-zinc-100">✕</button>
        </div>
      )}

      {/* In-Page Governance Status Change Panel (Zero Modal Architecture) */}
      {isStatusPanelOpen && (
        <Card className="p-5 border-zinc-700 bg-zinc-900/90 shadow-md space-y-4 animate-fadeIn">
          ...
          <form onSubmit={handleCommitStatusChange} className="space-y-4">
            {statusError && (
              <div className="p-3 rounded-md bg-red-950/40 border border-red-800/50 text-red-300 text-xs">
                {statusError}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  className="flex h-9 rounded-md border border-zinc-800 bg-zinc-950 px-3 py-1 text-xs text-zinc-100 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-zinc-400"
                >
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="h-9 px-3 flex items-center bg-zinc-950 border border-zinc-800 rounded-md text-zinc-300 font-mono">
                  {tenant.status}
                </div>
              </div>
            </div>
            ...
            {targetStatus === 'DEACTIVATED' && (
              <div className="p-3 rounded-md bg-amber-950/40 border border-amber-800/40 text-amber-300 text-xs flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          <Card className="p-5 space-y-4">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-400 border-b border-zinc-800 pb-3 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div>
                <span className="text-zinc-500 block mb-0.5">Tenant Classification</span>
                <strong className="text-zinc-200">{tenant.tenantType || 'COMMERCIAL_LAB'}</strong>
              </div>
              <div>
                <span className="text-zinc-500 block mb-0.5">GST Identification Number</span>
                <code className="text-zinc-300 font-mono">{tenant.gstNumber || 'Not provided'}</code>
              </div>
              <div>
                <span className="text-zinc-500 block mb-0.5">Registration Number</span>
                <span className="text-zinc-300 font-mono">{tenant.registrationNumber || 'Not provided'}</span>
              </div>
              <div>
              ...
              <div>
                <span className="text-zinc-500 block mb-0.5">Branch Capacity</span>
                <span className="text-zinc-300 flex items-center gap-1.5">
          ...
          <Card className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div className="col-span-2">
                <span className="text-zinc-500 block mb-0.5">Physical Address</span>
              ...
              <div>
                <span className="text-zinc-500 block mb-0.5">City & State</span>
              ...
              <div>
                ...
                <span className="text-zinc-300">{tenant.country || 'India'}</span>
              </div>
              <div>
                <span className="text-zinc-500 block mb-0.5">Timezone</span>
                <code className="text-zinc-300 font-mono">{tenant.timezone || 'Asia/Kolkata'}</code>
              </div>
              <div>
                <span className="text-zinc-500 block mb-0.5">Operational Currency</span>
                <code className="text-zinc-300 font-mono">{tenant.currency || 'INR'}</code>
              </div>
            </div>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card className="p-5 space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <strong className="text-zinc-200">{tenant.adminName || 'Admin'}</strong>
              </div>
              <div>
                <span className="text-zinc-500 block mb-0.5">Email Address</span>
                <div className="text-zinc-300 font-mono">{tenant.adminEmail}</div>
              </div>
            </div>
          </Card>

          {tenant.statusReason && (
            <Card className="p-4 space-y-1.5 border-zinc-800 bg-zinc-900/30">
          ...
          <Card className="p-5 space-y-2.5 text-xs text-zinc-400">
            <div className="flex items-center justify-between">
              <span>Onboarded At:</span>
              <span className="text-zinc-300 font-mono">{new Date(tenant.createdAt).toLocaleString()}</span>
            </div>
            <div className="flex items-center justify-between">
              <span>Last Modified:</span>
              <span className="text-zinc-300 font-mono">{new Date(tenant.updatedAt).toLocaleString()}</span>
            </div>
          </Card>
        </div>
      </div>
          ...
          <div className="divide-y divide-zinc-800/60">
            {organizations.map((org) => (
              <div key={org.id} className="py-3 flex items-center justify-between gap-4 first:pt-0 last:pb-0 text-xs">
                <div>
                  <div className="font-medium text-zinc-200">{org.name}</div>
                  <div className="text-[11px] text-zinc-400 font-mono flex items-center gap-2.5 mt-0.5">
                    <span className="text-zinc-300 font-semibold">{org.code}</span>
                ...
                <div className="flex items-center gap-3">
                  <Badge variant={org.status === 'ACTIVE' ? 'success' : 'outline'}>
                    {org.status}
                  </Badge>
          ...
          <div className="divide-y divide-zinc-800/60">
            {tenantAuditLogs.map((log) => (
              <div key={log.id} className="py-2.5 flex items-center justify-between gap-4 text-xs first:pt-0 last:pb-0">
                <div className="flex items-center gap-3">
                  <Badge variant={log.action.includes('DEACTIVATED') ? 'destructive' : 'default'}>
                    {log.action}
                  </Badge>
                  <span className="text-zinc-300 font-medium">{log.reason || log.action}</span>
                  <span className="text-zinc-500 text-[11px]">by {log.actorEmail}</span>
                </div>
